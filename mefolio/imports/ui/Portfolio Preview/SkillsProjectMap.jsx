...
export const SkillsProjectMap = ({ projects = [], viewportMode = "desktop" }) => {
  ...
  const { skills, projectNodes, graphHeight } = useMemo(() => {
    ...
    return {
      ...
      graphHeight:
        Math.max(Math.max(skillList.length, projectList.length) - 1, 0) * NODE_GAP +
        GRAPH_TOP_PADDING + GRAPH_BOTTOM_PADDING,
    };
  }, [projects]);
  ...
  const isNodeActive = (nodeType, node) => {
    ...
    if (hoveredNode.type === nodeType && hoveredNode.id === node.id) return true;
      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 id="skills-map-title" className="mt-2 text-3xl font-bold text-primary">
              Skills in practice
            </h2>
        ...
        <div className="overflow-x-auto rounded-2xl border border-line bg-surface-fill p-2 shadow-sm sm:p-3">
          ...
          >
            <defs>
              <pattern id="skills-map-grid" width="24" height="24" patternUnits="userSpaceOnUse">
                <path d="M 24 0 L 0 0 0 24" fill="none" className="stroke-line" strokeWidth="0.5" opacity="0.35" />
              </pattern>
            </defs>
            <rect width={GRAPH_WIDTH} height={graphHeight} rx="16" className="fill-background" />
            <rect width={GRAPH_WIDTH} height={graphHeight} rx="16" fill="url(#skills-map-grid)" />
            <line x1="520" y1="48" x2="520" y2={graphHeight - 32} className="stroke-line" strokeDasharray="4 8" />
            <text x="200" y="34" textAnchor="middle" className="fill-muted text-[11px] font-bold uppercase tracking-[0.16em]">
              Skills
            </text>
            <text x="760" y="34" textAnchor="middle" className="fill-muted text-[11px] font-bold uppercase tracking-[0.16em]">
              Projects
            </text>

            {skills.flatMap((skill) =>
              skill.projectIds.map((projectId) => {
                const project = projectNodes.find((node) => node.id === projectId);
                const active = isNodeActive("skill", skill) && isNodeActive("project", project);
            ...
            {skills.map((skill) => {
              ...
              return (
                <g
                  ...
                  onMouseEnter={() => setHoveredNode({ type: "skill", ...skill })}
                  onMouseLeave={clearHover}
                  onFocus={() => setHoveredNode({ type: "skill", ...skill })}
                  onBlur={clearHover}
                >
                  <circle cx={skill.x} cy={skill.y} r={skill.radius + 7} className="fill-accent1 opacity-10" />
                  <circle cx={skill.x} cy={skill.y} r={skill.radius} className="fill-accent1 stroke-background" strokeWidth="3" />
                  <text x={skill.x - skill.radius - 18} y={skill.y + 4} textAnchor="end" className="fill-primary text-[13px] font-bold">
                    {skill.label}
                  </text>
                </g>
              );
            ...
            {projectNodes.map((project) => {
              ...
              return (
                <g
                  ...
                  className={`cursor-pointer transition-opacity duration-200 ${active ? "opacity-100" : "opacity-30"}`}
                  onMouseEnter={() => setHoveredNode({ type: "project", ...project })}
                  onMouseLeave={clearHover}
                  onFocus={() => setHoveredNode({ type: "project", ...project })}
                  onBlur={clearHover}
                >
                  <circle cx={project.x} cy={project.y} r="25" className="fill-alt opacity-15" />
                  <circle cx={project.x} cy={project.y} r="18" className="fill-alt stroke-background" strokeWidth="3" />
                  <text x={project.x + 34} y={project.y + 4} className="fill-primary text-[13px] font-bold">
                    {project.label}
                  </text>
                </g>
              );
