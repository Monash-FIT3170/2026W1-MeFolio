  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-5">
        <span className="text-sm font-extrabold text-green-600">↗ {stat.change}</span>
      </div>
...
const VisitorCard = ({ visitor }) => {
  return (
    <div className="flex items-start gap-4 px-6 py-4 border-b border-gray-200 last:border-b-0">
      <div className={`w-2.5 h-2.5 rounded-full mt-2 shrink-0 ${visitor.active ? "bg-green-500" : "bg-gray-300"}`} />
      <div className="flex-1 min-w-0">
        ...
        <span className="text-xs text-gray-400">{visitor.location} · 2 min ago</span>
      </div>
    ...
    <>
      ...
      <section className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          ...
          <button className="text-indigo-600 font-bold text-sm bg-transparent border-none cursor-pointer">View all</button>
        </div>
